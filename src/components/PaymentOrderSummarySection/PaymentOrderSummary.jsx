    ...
    return (
        <div className="bg-[#FAF8F5] p-6 md:p-8 rounded-[20px] border border-[#EBEBEB] shadow-sm font-inter">
            ...
            <div className="space-y-4 mb-6">
                {cart.map((item, index) => (
                    <div key={index} className="flex justify-between items-center text-[14px] md:text-[16px] font-semibold text-[#000000]">
                        <span className="w-1/2 truncate pr-2">{item.title}</span>
                        <span className="w-1/4 text-center">x {item.quantity}</span>
                        <span className="w-1/4 text-right text-[#427C34]">{(item.price * item.quantity).toFixed(2)} SEK</span>
                    </div>
                ))}
            </div>

            <div className="border-t border-[#000000] opacity-30 mb-6"></div>

            {/* Subtotal & Delivery */}
            <div className="space-y-4 mb-6 text-[14px] md:text-[16px] font-normal text-[#808080]">
            ...
            <div className="flex justify-between items-center mb-8">
                <span className="font-bold text-[#000000] text-[24px] md:text-[28px]">Total</span>
                <span className="font-bold text-[#E6B220] text-[24px] md:text-[28px] xl:text-[32px]">{grandTotal.toFixed(2)} SEK</span>
            </div>

            {/* Scheduled Delivery Card */}
            <div className="border border-[#B3B3B3] rounded-[12px] p-3 md:p-4 flex items-center">
                <img src={DeliveryBikeIcon} alt="Delivery Bike Icon" className="w-[45px] h-[40px]" />
                <div className="ml-3 flex items-center flex-wrap gap-x-2 text-[12px] md:text-[14px] font-semibold text-[#000000]">
